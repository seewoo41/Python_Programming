...
start = time.time()
for i in range(10000):
    my_list.append("Hello")                 # 동일 리스트 객체에 append
...
s = "1410"
print(f"{s[0]}학년 {s[1]}반 {int(s[2])*10+int(s[3])}번")                  # ✅ "1학년 4반 10번" 출력

# 2️⃣ 학번을 슬라이싱만을 이용해서 학년, 반, 번호로 잘라내기
print(f"{s[0:1]}학년 {s[1:2]}반 {int(s[2:4])}번")                  # ✅ "1학년 4반 10번" 출력